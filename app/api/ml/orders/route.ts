...
function transformMLOrderData(mlOrder: any, integrationId: string) {
    return {
        integration_id: integrationId,
        ml_order_id: mlOrder.id.toString(),
        ml_pack_id: mlOrder.pack_id?.toString(),
        status: mlOrder.status,
        status_detail: mlOrder.status_detail?.description,
        date_created: mlOrder.date_created,
        date_closed: mlOrder.date_closed,
        date_last_updated: mlOrder.last_updated,
        total_amount: mlOrder.total_amount,
        currency_id: mlOrder.currency_id,
        paid_amount: mlOrder.paid_amount,
        buyer_id: mlOrder.buyer?.id?.toString(),
        buyer_nickname: mlOrder.buyer?.nickname,
        buyer_email: mlOrder.buyer?.email,
        buyer_first_name: mlOrder.buyer?.first_name,
        buyer_last_name: mlOrder.buyer?.last_name,
        buyer_phone: mlOrder.buyer?.phone?.area_code + mlOrder.buyer?.phone?.number,
        seller_id: mlOrder.seller?.id?.toString(),
        seller_nickname: mlOrder.seller?.nickname,
        quantity: mlOrder.order_items?.[0]?.quantity || 1,
        unit_price: mlOrder.order_items?.[0]?.unit_price,
        item_title: mlOrder.order_items?.[0]?.item?.title,
        item_category_id: mlOrder.order_items?.[0]?.item?.category_id,
        item_variation_id: mlOrder.order_items?.[0]?.item?.variation_id,
        item_variation_attributes: mlOrder.order_items?.[0]?.item?.variation_attributes || {},
        shipping_id: mlOrder.shipping?.id?.toString(),
        shipping_status: mlOrder.shipping?.status,
        shipping_mode: mlOrder.shipping?.shipping_mode,
        shipping_cost: mlOrder.shipping?.shipping_option?.cost,
        shipping_method: mlOrder.shipping?.shipping_option?.name,
        shipping_address: mlOrder.shipping?.receiver_address,
        tracking_number: mlOrder.shipping?.tracking_number,
        tracking_method: mlOrder.shipping?.tracking_method,
        payment_id: mlOrder.payments?.[0]?.id?.toString(),
        payment_method: mlOrder.payments?.[0]?.payment_method_id,
        payment_type: mlOrder.payments?.[0]?.payment_type_id,
        payment_status: mlOrder.payments?.[0]?.status,
        installments: mlOrder.payments?.[0]?.installments,
        mercadolibre_fee: mlOrder.fees?.mercadolibre_fee,
        shipping_fee: mlOrder.fees?.shipping_fee,
        financing_fee: mlOrder.fees?.financing_fee,
        context: mlOrder.context || {},
        tags: mlOrder.tags || [],
        feedback_rating: mlOrder.feedback?.rating,
        feedback_message: mlOrder.feedback?.message,
        feedback_date: mlOrder.feedback?.date_created,
