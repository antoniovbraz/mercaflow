...
O **MercaFlow** é uma plataforma SaaS enterprise-grade de gestão completa para vendedores do Mercado Livre, oferecendo integração nativa, sincronização automática de produtos/pedidos/mensagens, dashboard inteligente com métricas em tempo real e automação de processos. Nossa proposta é centralizar e otimizar toda a operação de vendas no ML, permitindo que PMEs gerenciem seus negócios de forma profissional, aumentando eficiência operacional em 60% e vendas em 25-40% nos primeiros 6 meses.
...
_"Sua vitrine profissional em 15 minutos, com IA que converte mais no ML"_
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em e-commerce que gera insights acionáveis",
      },
      {
        role: "user",
        content: `Analise este produto e gere 3 insights: ${JSON.stringify({
          product_data,
          market_data,
        })}`,
      },
...
serve(async (req) => {
  ...
  const impersonationToken = await supabase.auth.admin.generateLink({
    type: "magiclink",
    email: tenant_email,
    options: {
...
_Este documento representa o Single Source of Truth para o projeto Merca Flow e deve ser atualizado conforme evoluem as validações de mercado e decisões técnicas. Toda documentação segue padrões AI-friendly para desenvolvimento eficiente._
